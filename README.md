...
| | `xml.XmlNavigateCommand` | XML 特定要素の変換 (`IRule rule` が必要) | `new XmlNavigateCommand(new javax.xml.xpath.XPathExpression("//item/@id"), rule)`<br/><sup>※ XPath式は`javax.xml.xpath.XPathExpression`型で指定してください。</sup> |
