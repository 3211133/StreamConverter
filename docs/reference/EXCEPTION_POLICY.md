...
- 文脈情報（処理中レコード位置・rule 名等）を付加して投げる（文脈付加手段は `addSuppressed(StageFailureContext)` で確定済み — [§4.1.2](#412-文脈付加の手段) 参照）
...
- 吸収パターン: スキップ / 隔離 / リトライ
- 伝播時は**型を変えず文脈付加のみ**（例外型変換は A1/A2/A3 で確定済み — [§4.1.1](#411-例外型変換の許容範囲) 参照）
