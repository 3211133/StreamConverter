...
public class CommandFactory {
    ...
    public static <T extends IStreamCommand> T createWithLogging(Class<T> commandClass, Object... args) {
        try {
            ...
            log.info("Created command instance: {} with {} args", commandClass.getSimpleName(), args.length);
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T extends IStreamCommand> T createWithLogging(Class<T> commandClass, 
                                                                boolean enableDetailedLogging, 
                                                                Object... args) {
        try {
            T command = createInstance(commandClass, args);
            log.info("Created command instance: {} with {} args (detailed logging: {})", 
                    commandClass.getSimpleName(), args.length, enableDetailedLogging);
    ...
    public static IStreamCommand[] createPipelineWithLogging(CommandConfig... configs) {
        ...
        for (int i = 0; i < configs.length; i++) {
            ...
            try {
                ...
                log.error("Failed to create command {}/{}: {}", i + 1, configs.length, config.getDescription(), e);
    ...
    public static IStreamCommand[] createPipelineWithDetailedLogging(CommandConfig... configs) {
        ...
        for (int i = 0; i < configs.length; i++) {
            CommandConfig config = configs[i];
            try {
                IStreamCommand command = createWithLogging(config.getCommandClass(), true, config.getArgs());
                commands.add(command);
                log.debug("Added command {}/{} with detailed logging: {}", i + 1, configs.length, config.getDescription());
            } catch (Exception e) {
                log.error("Failed to create command {}/{}: {}", i + 1, configs.length, config.getDescription(), e);
                throw new RuntimeException("Pipeline creation failed at command " + (i + 1), e);
            }
        ...
        log.info("Created pipeline with detailed logging for {} commands successfully", commands.size());
        return commands.toArray(new IStreamCommand[0]);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    private static <T extends IStreamCommand> T createInstance(Class<T> commandClass, Object... args) throws Exception {
        // デフォルトコンストラクタを試す
        if (args.length == 0) {
     ...
     */
    @SuppressWarnings("unchecked")
    private static <T extends IStreamCommand> T createInstanceWithBestMatch(Class<T> commandClass, 
                                                                            Object[] args, 
        ...
        throw new NoSuchMethodException("No compatible constructor found for " + commandClass.getSimpleName());
    }
