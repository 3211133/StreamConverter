 ...
 * 自動ログ出力機能のデモンストレーション
 * 
 * <p>このクラスは、自動ログ出力機能の使用方法を示すサンプルです。
 * 以下の機能をデモンストレーションします：
 * - AbstractStreamCommandの自動ログ出力
 * - LoggingDecoratorによる詳細ログ出力
 * - CommandFactoryによるログ機能付きコマンド生成
 * - パイプライン全体の自動ログ出力
 */
public class AutoLoggingDemo {
    ...
    private static void demonstrateCommandFactory() throws IOException {
        ...
        IStreamCommand factoryCommand = CommandFactory.createWithLogging(XmlNavigateCommand.class, "users/user/name");
    ...
    private static void demonstrateErrorLogging() {
        ...
        try {
            ...
            // 詳細ログ付きでコマンドを作成
            IStreamCommand baseCommand = CommandFactory.createWithLogging(XmlNavigateCommand.class, "invalid/path");
     ...
     */
    private static String processData(String inputData, IStreamCommand[] commands) throws IOException {
