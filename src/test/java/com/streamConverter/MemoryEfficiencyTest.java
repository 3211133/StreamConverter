...
class MemoryEfficiencyTest {
    ...
    void testLargeStreamMemoryEfficiency() throws IOException {
        ...
        // 3つのコマンドでパイプライン処理
        StreamConverter converter = new StreamConverter(new IStreamCommand[] {
        ...
        assertTrue(memoryIncrease < maxAcceptableMemory, 
            "Memory usage too high: " + (memoryIncrease / 1024 / 1024) + "MB > " + (maxAcceptableMemory / 1024 / 1024) + "MB");
    }
    ...
    void testSingleCommandMemoryEfficiency() throws IOException {
        ...
        // 単一コマンド（最適パス）
        StreamConverter converter = new StreamConverter(new IStreamCommand[] {new SampleStreamCommand("single")});
    ...
    void test1GBStreamMemoryEfficiency() throws IOException {
        ...
        try { Thread.sleep(100); } catch (InterruptedException e) { Thread.currentThread().interrupt(); } // GC完了待機
        long beforeMemory = runtime.totalMemory() - runtime.freeMemory();
        ...
        assertTrue(memoryIncrease < maxAcceptableMemory, 
            "1GB processing memory usage too high: " + (memoryIncrease / 1024 / 1024) + "MB > " + (maxAcceptableMemory / 1024 / 1024) + "MB");
        
        // パフォーマンス検証 - 最低1MB/s以上のスループット
        assertTrue(throughputMBps > 1.0, 
