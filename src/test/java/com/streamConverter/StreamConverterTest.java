...
class StreamConverterTest {
    ...
    void setUp() {
        // テスト前の準備
        validCommands = new IStreamCommand[] {
            new SampleStreamCommand("test1"),
            new SampleStreamCommand("test2")
        };
    ...
    void testRunWithValidStreams() throws IOException {
        ...
        try (InputStream inputStream = new ByteArrayInputStream(testInput.getBytes(StandardCharsets.UTF_8));
    ...
    void testMultipleCommands() throws IOException {
        // 複数コマンドを使用した場合のテスト
        IStreamCommand[] commands = new IStreamCommand[] {
            new SampleStreamCommand("1"),
            new SampleStreamCommand("2"),
            new SampleStreamCommand("3")
        };
