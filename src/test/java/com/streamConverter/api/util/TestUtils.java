...
public class TestUtils {
  ...
  public static void waitUntil(
      Supplier<Boolean> condition, Duration timeout, Duration pollInterval) {
    Awaitility.await().atMost(timeout).pollInterval(pollInterval).until(condition::get);
  }
   ...
   */
  public static <T> T waitForCompletion(CompletableFuture<T> future, Duration timeout)
      throws Exception {
    return Awaitility.await()
        .atMost(timeout)
        .until(() -> future.isDone() ? future.get() : null, result -> result != null);
  }
   ...
   * @return 最終的なステータス
   */
  public static String waitForBatchStatus(
      Supplier<String> statusSupplier, String expectedStatus, Duration timeout) {
    return Awaitility.await()
        .atMost(timeout)
  ...
  public static void waitForAsyncStart(Supplier<Boolean> condition, Duration timeout) {
    Awaitility.await().atMost(timeout).pollInterval(Duration.ofMillis(50)).until(condition::get);
  }
