...
class ConvertTest {
    ...
    void testExecuteWithNullOutputStream() {
        ...
        convert command = new convert("UTF-8", "UTF-16");
        InputStream inputStream = new ByteArrayInputStream(TEST_STRING.getBytes(StandardCharsets.UTF_8));
    ...
    @Disabled("This test is disabled because java.io.UnsupportedEncodingException thrown.")
    void testExecuteWithInvalidInputCharset() {
        // 無効な入力文字コードでのexecuteメソッドテスト
        convert command = new convert("INVALID-CHARSET", "UTF-8");
        InputStream inputStream = new ByteArrayInputStream(TEST_STRING.getBytes(StandardCharsets.UTF_8));
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
    ...
    void testExecuteWithInvalidOutputCharset() {
        // 無効な出力文字コードでのexecuteメソッドテスト
        convert command = new convert("UTF-8", "INVALID-CHARSET");
        InputStream inputStream = new ByteArrayInputStream(TEST_STRING.getBytes(StandardCharsets.UTF_8));
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
    ...
    void testJapaneseCharacterConversion() throws IOException {
        ...
        try (InputStream inputStream = new ByteArrayInputStream(japaneseText.getBytes(StandardCharsets.UTF_8));
             ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
