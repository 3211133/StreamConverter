 ...
 * JSON Schema検証では構造全体の検証が必要なため（例: 配列の要素数制約、相互参照）、 完全なストリーミング処理は技術的に不可能です。 本実装ではJackson streaming
 * APIを使用してJSON解析を行いますが、 検証のためにDOM構築が必要となり、全データがメモリに読み込まれます。
 *
 * <p><strong>メモリ使用について:</strong><br>
 * 大容量のJSONファイルを処理する場合、ファイルサイズに応じたメモリが必要です。 メモリ制約がある環境では、ファイルサイズに注意してください。
 *
 * <p>使用例:
 *
 * <pre>
