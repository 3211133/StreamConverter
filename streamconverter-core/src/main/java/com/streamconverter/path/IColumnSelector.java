 ...
 *
 * <p><b>実装規約:</b> 戻り値は null であってはならない。該当列がない場合は空リストを返すこと。 インデックスは 0 始まりの整数として返すこと。
 *
 * <p>使用例:
 *
 * <pre>{@code
...
public interface IColumnSelector {
   ...
   *
   * @param headers CSV列ヘッダー配列。null の場合は空リストを返すこと。
   * @return 対象列インデックスのリスト（空の場合は該当列なし）。null を返してはならない。
   */
   ...
   *
   * @param totalColumns 総列数（0 以上を期待する。負値の場合は空リストを返すこと）。
   * @return 対象列インデックスのリスト（空の場合は該当列なし）。null を返してはならない。
   */
